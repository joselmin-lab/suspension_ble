// Legacy: 1 motor, comandos por letras.
// OJO: el comando 'D' estaba duplicado en el original (conflicto entre mover -2 y Kd+).
...
void setDirection(bool dir) { digitalWrite(DIR_PIN, dir); }
void enableMotor(bool en) { digitalWrite(EN_PIN, !en); }
...
void motorControl() {
  ...
  if (now - last_step_time >= step_interval) {
    last_step_time = now;
    stepMotor();
    current_steps += dir ? 1 : -1;
  }
}

void moveStages(int stages) {
  long move_steps = (TARGET_INCREMENT / STEP_ANGLE) * stages;
...
void parseCommand(char c) {
  switch (c) {
    ...
    // NOTA: en el original era 'D' dos veces. Aquí dejamos solo mover -2 para compilar.
    ...
    case 'K': Kd += 0.5; break; // cambié comando para evitar conflicto
    case 'k': Kd -= 0.5; break;
  }
}
...
void loop() {
  while (SERIAL_BT.available()) {
    parseCommand((char)SERIAL_BT.read());
  }
  motorControl();
}